...
// All menu links to test from Footer and Header
const testLinks = {
  'en-us': {
    navbar: [
      '/en-us/',
...
async function testLinks(port = 3000) {
  const results = {
    total: 0,
    passed: 0,
    failed: 0,
  ...
  for (const [lang, links] of Object.entries(testLinks)) {
    console.log(`\n📋 Testing ${lang.toUpperCase()} links...`);

    // Test navbar links
    if (links.navbar) {
      for (const url of links.navbar) {
        results.total++;
        const status = await testUrl(url, port);
        if (status === 200) {
          results.passed++;
          ...
          results.failed++;
          process.stdout.write('✗');
          results.errors.push({ lang, url, status });
        }
      }
    }
      ...
      for (const [category, urls] of Object.entries(links.footer)) {
        for (const url of urls) {
          ...
          if (status === 200) {
            ...
            results.failed++;
            process.stdout.write('✗');
            results.errors.push({ lang, url, status, category });
          }
        }
      }
    }
  ...
  if (results.errors.length > 0) {
    ...
    console.log(`\n✅ All menu links return 200 status!\n`);
    process.exit(0);
  }
}

function testUrl(path, port) {
...
server.stdout.on('data', (data) => {
  ...
  if (output.includes('ready') || output.includes('Local:')) {
    if (!serverReady) {
      serverReady = true;
      setTimeout(() => testLinks(4321), 1000);
    }
  }
});
